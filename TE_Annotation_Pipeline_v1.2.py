...
def step2_ltrfinder(cwd, genome, threads, logger, resume=False):
    ...
    run_cmd(f"LTR_FINDER_parallel -seq {genome} -threads {threads} -harvest_out -size 1000000 -time 300", logger, work_dir=step_dir)
    create_checkpoint(step_dir, 2)

def step3_ltr_retriever(cwd, genome, logger, threads, mutation_rate, resume=False):
    ...
    run_cmd(f"LTR_retriever -genome {genome} -inharvest raw_LTR.scn -threads {threads} -u {mutation_rate}", logger, work_dir=step_dir)
...
def step4_repeatmodeler(cwd, genome, logger, threads=32, database_name="default_db", resume=False):
    ...
    run_cmd(f"RepeatMasker -pa {threads} -no_is -e ncbi -lib LTR.lib -dir 01.RM-1 {genome}", logger, work_dir=step_dir)
    ...
    run_cmd(f"BuildDatabase -name {database_name} -engine ncbi {masked_genome}", logger, work_dir=step_dir)
...
def step6_merge_libs(cwd, genome, sp, logger, resume=False):
    ...
    run_cmd(f"cat {modeler_lib} {sp_db} {ltr_lib_path} > allRepeats_redundancy.lib", logger, work_dir=step_dir)
    run_cmd(f"vsearch --cluster_fast allRepeats_redundancy.lib --id 0.97 --centroids allRepeats.lib", logger, work_dir=step_dir)
...
def step7_repeatmasker_final(cwd, genome, logger, threads=32, resume=False):
    ...
    run_cmd(f"RepeatMasker -e ncbi -gff -xsmall -html -norna -source -no_is -pa {threads} -s -lib allRepeats.lib genome.fasta", logger, work_dir=step_dir)
...
def step9_trf(cwd, logger, resume=False):
    ...
    run_cmd(f"cp {masked_genome} {step_dir}", logger, work_dir=step_dir)
    run_cmd(f"trf genome.hardmasked.fa 2 7 7 80 10 50 2000 -h -d", logger, work_dir=step_dir, allowed_returncodes=[0,1])
    trf_dat = os.path.join(step_dir, f"{masked_genome}.2.7.7.80.10.50.2000.dat")
...
def main():
    ...
    parser.add_argument("--mutation_rate", type=float, help="Mutation rate of this genomic group, default = 1.01e-9", default=1.01e-9)
    ...
    required_tools = [
        ...
        "RepeatMasker", "famdb.py", "bedtools", "trf", "vsearch"
    ]
    try:
    ...
    try:
        ...
        step3_ltr_retriever(cwd, genome, threads=args.threads, mutation_rate=args.mutation_rate, logger=logger, resume=args.resume)
